...
class CustomStat {
    ...
    #displayStats() {
        ...
        if ((att1 !== null && att2 !== null) && (att3 === null || this.#definedAffixMode)) {
            ...
            console.log(`Elapsed time: ${elapsedTime.toFixed(4)} seconds`);
        } else {
            if (att4 === null) {
    ...
    #setValue(lblAttr, cboValue) {
        ...
        for (let i = 0; i < Attribute.STATS.length; i++) {
            const attributes = Attribute.STATS[i];
    ...
    setAsMemoryAddress(objArtifactPiece) {
        if (objArtifactPiece instanceof ArtifactPiece) {
            this.#objArtifactPiece = objArtifactPiece;
        } else {
            throw new TypeError("Not an instance of ArtifactPiece Class");
        }
    }
}